...
export const ListContacts = () => {
  ...
  const la = async () => {
    try {
      ...
      const customers = await quick.getContacts({}, { client: ctx.zora });
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <button onClick={e => la()}>Get Contacts</button>
    </div>
  );
};
...
export const GetContact = () => {
  ...
  const la = async () => {
    try {
      ...
      const quick = getQuickstartAPIReference();
      const acct = await quick.getContact(id, {}, { client: ctx.zora });
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      />
      &nbsp;
      <button onClick={e => la()}>Get Contact</button>
    </div>
  );
};
...
export const DeleteContact = () => {
  ...
  const la = async () => {
    try {
      ...
      const quick = getQuickstartAPIReference();
      const acct = await quick.deleteContact(id, { client: ctx.zora });
      ctx.setMsgs(JSON.stringify(acct, null, 2));
    } catch (e) {
      ctx.setMsgs('ERROR:\n' + JSON.stringify(e, null, 2));
    }
